...
| **Minimal default base (dev/test shortcut)** | `images/base/Containerfile` | A minimal rootfs for dev/test use via `images/AGENT-REBUILD.md` rootfs shortcut. **Not** the `nexus3-agent-base` production image. |
| **`nexus3-agent-base` production image** | `internal/test/selfhost/baseimage_agent.go` (`generateAgentContainerfile`) | The full self-hosting base — Go + Node.js + Claude Code + gh + seeded module cache — built by `rebuild-base.sh --image`. This is what `sandbox create --agent` uses. |
...
## Default base (`images/base/`) — dev/test shortcut only

> **Note:** `images/base/Containerfile` is a **dev/test shortcut** (minimal rootfs,
> no Node.js, no Claude Code). It is NOT the source of the `nexus3-agent-base`
> production image. The production image is generated by `generateAgentContainerfile()`
> in `internal/test/selfhost/baseimage_agent.go` and built by `rebuild-base.sh --image`.
> Edits to `images/base/Containerfile` have no effect on `nexus3-agent-base`.
